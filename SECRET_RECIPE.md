...
Six rules. If you violate them the rest collapses, and they are precisely the rules a capable coding agent breaks on instinct. Most of the build is one of these made executable.
...
- **`/durable-crons`** — register the recurring maintenance checks (Phase 7 hygiene nags, drift checks) as persistent scheduled tasks so upkeep doesn't depend on anyone remembering.
